...
- **A start script and the program's own checksum, inside the archive.** Every
  ...
  nothing on screen for that whole wait. `IRIS_SKIP_VERIFY=1` skips the
...
- **The archive unpacks to a folder named after the tool.** It was
  ...
  the same program. It is now `Iris/`.
...
- **The release smoke test now saves a file.** `--version` exits during
  ...
  release fails if a Linux bundle does not come up on x11 — then reads a
  recipient out of a spreadsheet, writes a message with an attachment and
  reads both back. The
...
- **§11 of `COMMERCIAL-LICENSE.md` describes what a redistributor receives.**
  ...
  redistributor actually ships is a frozen bundle of 109 native libraries,
...
- **A GPL-3.0 library was being shipped inside archives offered for commercial
  ...
  only that module does, and Iris never reads a line from an interactive
  prompt. It and `rlcompleter` are excluded from the build in both `build.py` and `Iris.spec`, `libtinfo`
