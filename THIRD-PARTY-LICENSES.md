...
Iris is licensed **AGPL-3.0-or-later** (see [LICENSE](LICENSE)), with a
commercial licence available separately (see
[COMMERCIAL-LICENSE.md](COMMERCIAL-LICENSE.md)). That covers the code in this
repository. It does not cover the code Iris is built on, and a
downloadable release is mostly that other code: a Linux build contains
82 native binaries and not one of them was written for Iris.
Iris's own code travels through them as Python bytecode.
...
## What Iris depends on directly

Seven packages, declared in [`requirements.txt`](requirements.txt), with the
licence each one's own metadata states:

| Package | Version built | Licence (from its metadata) | What Iris uses it for |
|---|---|---|---|
| pypdf | 6.16.2 | `BSD-3-Clause` | reading recipient lists out of PDFs |
| openpyxl | 3.1.5 | `MIT` | reading `.xlsx` and `.xlsm` |
| xlrd | 2.0.2 | `BSD-3-Clause` | reading legacy `.xls`, which openpyxl does not support |
| python-docx | 1.2.0 | `MIT` | reading `.docx` |
| Pillow | 12.3.0 | `MIT-CMU` | required by ttkbootstrap to build its theme |
...
python-docx brings `lxml` (BSD-3-Clause), which is the only transitive
dependency with native code of its own. On Windows, `pywin32` is added for
`.msg` generation through Outlook; it is PSF-style and absent from the other
two platforms' archives.

Every one of these imposes attribution and nothing more. **Nothing in the
closure is copyleft**, and that is a choice rather than luck: PDF reading used
to use PyMuPDF, dual-licensed by Artifex under AGPL-3.0 or a paid commercial
licence of its own. A commercial licence to Iris removes *Iris's* copyleft
obligation and could never remove Artifex's, so a closed-source product
shipping PDF support would have needed a second commercial licence from a third
party. The reader uses `pypdf` instead, which extracts the same page text with
nothing attached.
...
Iris never reads a line from an interactive prompt. It and `rlcompleter` are
excluded in [`build.py`](build.py) and [`Iris.spec`](Iris.spec), and
[`tests/test_packaging.py`](tests/test_packaging.py) pins both so the exclusion
cannot silently come back. `libtinfo` left with it.

## Licence texts travel with the build

The v2.3.0 archives contained one executable and nothing else. A recursive
...
system libraries require in stronger terms, and which Iris's own AGPL
requires as well.
...
### Linux — 82 native binaries

| Component | Files | Licence | Evidence |
|---|---|---|---|
| `CPython` (cpython) | 24 | PSF-2.0 | the Python Software Foundation License, version 2 |
...
`licenses/` for that reason. Nothing else used only to build Iris appears
...
- **Only native binaries are inventoried.** Python code shipped as bytecode is
  ...
  assembled per distribution rather than per binary. Every distribution in an Iris build declares a permissive licence, so
...
python tools/licence_inventory.py --bundle linux=build/Iris --markdown out.md
```
