#!/bin/sh
# Start Iris, after checking that the executable is the one this archive was
# built with.
#
...
if [ "${IRIS_SKIP_VERIFY:-}" = "1" ]; then
    echo "$APP: checksum verification skipped (IRIS_SKIP_VERIFY=1)" >&2
elif [ ! -f "$sums" ]; then
