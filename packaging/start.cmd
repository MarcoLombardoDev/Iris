@echo off
rem Start Iris, after checking that the executable is the one this archive
rem was built with.
rem
rem The archive ships Iris.exe.sha256 beside the executable. This script
...
if "%IRIS_SKIP_VERIFY%"=="1" (
    echo %APP%: checksum verification skipped ^(IRIS_SKIP_VERIFY=1^) 1>&2
    goto :launch
)
...
set "_LAUNCH_TIMEOUT=%IRIS_LAUNCH_TIMEOUT%"
