...
exception** — so every Linux archive Iris has published contains a GPL-3
...
``libpython`` does not link it; only that module does. Nothing in Iris
...
BUILD_INPUTS = ['build.py', 'Iris.spec']
...
class TestLooksLikeTheOthers:
    ...
    def test_the_theme_is_chosen_by_trying_not_by_looking_it_up(self):
        ...
        source = (REPO / "iris/gui.py").read_text(encoding="utf-8")
        loops = [
            node
    ...
    def test_the_theme_resolves_to_the_same_palette(self):
        ...
        try:
            try:
                ...
                # gui.py wraps exactly this call and falls back to plain ttk
                # with a logged warning, so a machine like that gets Iris
                # without the bootstrap theme rather than no Iris. The test was
...
class TestInterfaceFont:
    ...
    def test_the_family_resolves_to_something_real(self):
        ...
        from iris.gui import ui_font_family
        try:
            root = tk.Tk()
        except tk.TclError as exc:
            pytest.skip(f"no display: {exc}")
