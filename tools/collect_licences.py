...
to accompany the object code, and Iris's own AGPL requires the same.
...
Iris is built with ``--onefile``, so unlike Orion there is no bundle
...
def collect(repo: str, staging: str) -> str:
    ...
    index = [
        ...
        "",
        "Iris itself is licensed AGPL-3.0-or-later; the full text is in",
        "`Iris-LICENSE.txt`. A commercial licence, without the AGPL's",
        ...
        "## Iris",
        "",
        "- `Iris-LICENSE.txt` — GNU Affero General Public License v3.0",
        "",
    ]

    shutil.copyfile(
        os.path.join(repo, "LICENSE"), os.path.join(staging, "Iris-LICENSE.txt")
    )
    ...
    index += [
        ...
        "`Iris-LICENSE.txt` and the sources in the repository cover",
        "Iris's own code.",
        "",
    ]
